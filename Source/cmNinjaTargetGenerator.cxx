...
void cmNinjaTargetGenerator::OpenDependInfoFileStream() 
{
  // Compute cmake DependInfo.cmake location
  cmLocalNinjaGenerator* lg = this->GetLocalGenerator();

  if (!this->DependInfoFileStream)
  {
    this->DependInfoFileName = lg->GetRelativePath(lg->GetTargetDirectory(*this->Target));
    this->DependInfoFileName += "/";
    this->DependInfoFileName += cmDependInfo::DEPEND_INFO_FILE;

    this->DependInfoFileStream = new cmGeneratedFileStream(
...
void cmNinjaTargetGenerator::CloseDependInfoFileStream() 
{
  if (this->DependInfoFileStream)
  {
    delete this->DependInfoFileStream;
    this->DependInfoFileStream = 0;
  }
  else
  {
    cmSystemTools::Error("Cmake DependInfo.cmake file stream was not open.");
  }
}

cmGeneratedFileStream& cmNinjaTargetGenerator::GetDependInfoFileStream() const
{
  return *this->DependInfoFileStream;
}
...
::WriteObjectBuildStatements()
{
  ...
  for(std::vector<cmSourceFile*>::const_iterator
    ...
    {
    this->ObjectPaths.push_back(this->GetSourceFilePath(*si));
    }
  for(std::vector<cmSourceFile*>::const_iterator
  ...
  for(std::vector<std::string>::iterator oi = objs.begin();
      oi != objs.end(); ++oi)
    {
    this->ObjectPaths.push_back(ConvertToNinjaPath(oi->c_str()));
    }
  }
...
::WriteObjectBuildStatement(cmSourceFile* source)
{
  ...
  cmNinjaDeps outputs;
  std::string objectFileName = this->GeneratorTarget->GetObjectFilePath(source);
  outputs.push_back(objectFileName);
  // Add this object to the list of object files.
  this->ObjectPaths.push_back(objectFileName);
...
void cmNinjaTargetGenerator::WriteDependencyScanBuildStatement()
{
  ...
  cmLocalGenerator* localGenerator = this->GetLocalGenerator();

  //
  // Add a "depends" target to run cmake_depends
  //
  ...
  //
  cmNinjaVars vars;
  vars["HOME_SRC_DIR"] = localGenerator->Convert(
    this->Makefile->GetHomeDirectory(),
    cmLocalGenerator::FULL, cmLocalGenerator::SHELL);
  vars["START_SRC_DIR"] = localGenerator->Convert(
    this->Makefile->GetStartDirectory(),
    cmLocalGenerator::FULL, cmLocalGenerator::SHELL);
  vars["HOME_OUT_DIR"] = localGenerator->Convert(
    this->Makefile->GetHomeOutputDirectory(),
    cmLocalGenerator::FULL, cmLocalGenerator::SHELL);
  vars["START_OUT_DIR"] = localGenerator->Convert(
