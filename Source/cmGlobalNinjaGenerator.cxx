...
const char* cmGlobalNinjaGenerator::CMAKE_FILE = "Makefile.cmake";
...
void cmGlobalNinjaGenerator::OpenCmakeFileStream()
{
  // Compute Ninja's build file path.
  std::string cmakeFilePath =
    this->GetCMakeInstance()->GetHomeOutputDirectory();
  cmakeFilePath += cmake::GetCMakeFilesDirectory();
  cmakeFilePath += "/";
  cmakeFilePath += cmGlobalNinjaGenerator::CMAKE_FILE;

  // Get a stream where to generate things.
  if (!this->CmakeFileStream)
  {
    this->CmakeFileStream = new cmGeneratedFileStream(cmakeFilePath.c_str());
    if (!this->CmakeFileStream)
    {
      // An error message is generated by the constructor if it cannot
      // open the file.
  ...
  // Write the do not edit header.
  this->WriteDisclaimer(*this->CmakeFileStream);
}

void cmGlobalNinjaGenerator::CloseCmakeFileStream()
{
  if (this->CmakeFileStream)
  {
    delete this->CmakeFileStream;
    this->CmakeFileStream = 0;
  }
  else
  {
    cmSystemTools::Error("Cmake file stream was not open.");
  }
}
